...
function subscriptionsModalController(
    ...
    //vm.subLoadSuccess can also do a CREATE, if no sub exists for that Team/User ID.
    function _renderSubscriptionByUserOrTeamId() {

        if (vm.selectedUser) {
            vm.$subscriptionsService.getSubscriptionByUserId(vm.selectedUser.userID, vm.subLoadSuccess, vm.subError);
            console.log(vm.selectedUser);

        }
        if (vm.selectedTeam) {
            vm.$subscriptionsService.getSubscriptionByTeamId(vm.selectedTeam.teamsId, vm.subLoadSuccess, vm.subError);
    ...
    function _subLoadSuccess(data, status, settings) {
        ...
        if (mySub) {
            ...
            if (mySub.userId) {
                ...
                if (vm.selectedUser.userSubscription.startDate == '0001-01-01T00:00:00') {
                    mySub.startDate = null;
                }

                if (vm.selectedUser.userSubscription.expireDate == '0001-01-01T00:00:00') {
                ...
                if (vm.selectedTeam.userSubscription.startDate == '0001-01-01T00:00:00') {
                    mySub.startDate = null;
                }

                if (vm.selectedTeam.userSubscription.expireDate == '0001-01-01T00:00:00') {
                    mySub.expireDate = null;
                }

                console.log(vm.selectedTeam.userSubscription);
            ...
            if (vm.selectedUser) {
                vm.userType = 'user';
                vm.$usersService.loadUsers(vm.loadUserNameSuccess, vm.subError);
                vm.userTeam = vm.selectedUser.userID;
            }

            if (vm.selectedTeam) {
                vm.userType = 'team';
                vm.$teamsService.onTeamsSelectAll(vm.loadTeamTitleSuccess, vm.subError);
                vm.userTeam = vm.selectedTeam.teamsId;
            }
        }
    ...
    function _selectedPremium() {
        ...
        if (vm.currentSub.subscriptionType == 'Premium') {
        ...
        if (vm.currentSub.subscriptionType == 'Trial') {
